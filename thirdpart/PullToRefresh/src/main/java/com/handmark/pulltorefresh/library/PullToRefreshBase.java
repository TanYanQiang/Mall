...
import com.handmark.pulltorefresh.library.internal.UserDefinedLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private UserDefinedMode mUserDefinedMode = UserDefinedMode.NO_DEFINE;

	private LoadingLayout mHeaderLayout;
	private LoadingLayout mFooterLayout;

    private boolean showUserDefined=false;
    private LoadingLayout mHeaderNoValidMoreLayout;
	private LoadingLayout mFooterNoValidMoreLayout;
	...
	protected LoadingLayoutProxy createLoadingLayoutProxy(final boolean includeStart, final boolean includeEnd) {
		...
		if (includeStart && mMode.showHeaderLoadingLayout()) {
			proxy.addLayout(getmHeaderLayout());
		}
		if (includeEnd && mMode.showFooterLoadingLayout()) {
			proxy.addLayout(getmFooterLayout());
		}
	...
	protected void onPullToRefresh() {
		switch (mCurrentMode) {
			case PULL_FROM_END:
				getmFooterLayout().pullToRefresh();
				break;
			case PULL_FROM_START:
				getmHeaderLayout().pullToRefresh();
				break;
			default:
				// NO-OP
				break;
    ...
    private LoadingLayout getmHeaderLayout() {
        if(showUserDefined&&mHeaderNoValidMoreLayout!=null){
            return mHeaderNoValidMoreLayout;
        }
        return mHeaderLayout;
    }

    private LoadingLayout getmFooterLayout() {
        if(showUserDefined
                &&mFooterNoValidMoreLayout!=null){
            return mFooterNoValidMoreLayout;
        }
	...
	protected void onRefreshing(final boolean doScroll) {
		if (mMode.showHeaderLoadingLayout()) {
			getmHeaderLayout().refreshing();
		}
		if (mMode.showFooterLoadingLayout()) {
			getmFooterLayout().refreshing();
		}
	...
	protected void onReleaseToRefresh() {
		switch (mCurrentMode) {
			case PULL_FROM_END:
				getmFooterLayout().releaseToRefresh();
				break;
			case PULL_FROM_START:
				getmHeaderLayout().releaseToRefresh();
				break;
			default:
				// NO-OP
				break;
	...
	protected void onReset() {
		...
		getmHeaderLayout().reset();
		getmFooterLayout().reset();
		smoothScrollTo(0);
	}
	...
	protected final void setHeaderScroll(int value) {
		...
		if (mLayoutVisibilityChangesEnabled) {
			if (value < 0) {
				getmHeaderLayout().setVisibility(View.VISIBLE);
			} else if (value > 0) {
				getmFooterLayout().setVisibility(View.VISIBLE);
			} else {
				getmHeaderLayout().setVisibility(View.INVISIBLE);
				getmFooterLayout().setVisibility(View.INVISIBLE);
			}
		}
	...
	protected void updateUIForMode() {
        ...
        if(mHeaderNoValidMoreLayout!=null){
            if (this == mHeaderNoValidMoreLayout.getParent()) {
                removeView(mHeaderNoValidMoreLayout);
            }
        }
        if (mMode.showHeaderLoadingLayout()) {
            addViewInternal(getmHeaderLayout(), 0, lp);
        }
        ...
        if(mFooterNoValidMoreLayout!=null){
            if(this==mFooterNoValidMoreLayout.getParent()){
                removeView(mFooterNoValidMoreLayout);
            }
        }
        if(mMode.showFooterLoadingLayout()){
            addViewInternal(getmFooterLayout(),lp);
        }
	...
	private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrNoValidMoreUserDefinedMode)){
            mUserDefinedMode = UserDefinedMode.mapIntToValue(a.getInteger(
                    R.styleable.PullToRefresh_ptrNoValidMoreUserDefinedMode, 0));
        }
		// We need to create now layouts now
        mHeaderLayout = createLoadingLayout(context, Mode.PULL_FROM_START, a);
        mFooterLayout = createLoadingLayout(context, Mode.PULL_FROM_END, a);
        if(mUserDefinedMode==UserDefinedMode.NO_DEFINE
                ||mUserDefinedMode==UserDefinedMode.FOOTER) {
            mHeaderNoValidMoreLayout=null;
        }else{
            mHeaderNoValidMoreLayout=createUserDefinedHeaderLayout(context,Mode.PULL_FROM_START,a);
        }
        if(mUserDefinedMode==UserDefinedMode.NO_DEFINE
                ||mUserDefinedMode==UserDefinedMode.HEADER){
            mFooterNoValidMoreLayout=null;
        }else{
            mFooterNoValidMoreLayout=createUserDefinedFooterLayout(context, Mode.PULL_FROM_END, a);
        }
    ...
    private LoadingLayout createUserDefinedHeaderLayout(Context context, Mode mode, TypedArray a) {
        if(a.hasValue(R.styleable.PullToRefresh_ptrUD_header_layout)) {
            return new UserDefinedLayout(context, mode, getPullToRefreshScrollDirection(), a, UserDefinedMode.HEADER);
        }else{
            return null;
        }
    }

    private LoadingLayout createUserDefinedFooterLayout(Context context, Mode mode, TypedArray a) {
        if(a.hasValue(R.styleable.PullToRefresh_ptrUD_footer_layout)){
            return new UserDefinedLayout(context, mode, getPullToRefreshScrollDirection(), a,UserDefinedMode.FOOTER);
        }else{
            return null;
        }
    }
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			switch (mCurrentMode) {
				case PULL_FROM_END:
					getmFooterLayout().onPull(scale);
					break;
				case PULL_FROM_START:
				default:
					getmHeaderLayout().onPull(scale);
					break;
			}
    ...
    public static enum UserDefinedMode{
        NO_DEFINE,
        HEADER,
        FOOTER,
        BOTH;

        public static UserDefinedMode mapIntToValue(int modeInt) {
            switch (modeInt) {
                case 0x0:
                default:
                    return NO_DEFINE;
    ...
    public void setShowUserDefined(boolean showUserDefined) {
        if(showUserDefined!=this.showUserDefined) {
            if (DEBUG) {
                Log.d(LOG_TAG, "Setting mode to: " + showUserDefined);
            }
            this.showUserDefined = showUserDefined;
            updateUIForMode();
        }
    }
}
